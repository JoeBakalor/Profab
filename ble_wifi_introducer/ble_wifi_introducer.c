...
typedef PACKED struct
{
    ...
    uint16_t        scan_result_characteristic_client_configuration;
...
static const uint8_t wifi_introducer_gatt_server_database[]=
{
    ...
    PRIMARY_SERVICE_UUID128( HANDLE_WIFI_INTRO_SENS_NW_SERVICE, UUID_WIFI_INTRODUCER_SERVICE ),
        ...
        CHARACTERISTIC_UUID128_WRITABLE( HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_PASSPHRASE, HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_PASSPHRASE_VAL,
            ...
            /* Declare characteristic used to notify/indicate scan results*/
         CHARACTERISTIC_UUID128( HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_SCAN_RESULT, HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_SCAN_RESULT_VAL,
            UUID_WIFI_INTRODUCER_CHARACTERISTIC_SCAN_RESULT, LEGATTDB_CHAR_PROP_READ | LEGATTDB_CHAR_PROP_NOTIFY | LEGATTDB_CHAR_PROP_INDICATE, LEGATTDB_PERM_READABLE ),
            ...
            /* Declare client characteristic configuration descriptor
                      ...
                      * for bonded devices.  Setting value to 1 tells this application to send notification
                      * when value of the characteristic changes.  Value 2 is to allow indications. */
          CHAR_DESCRIPTOR_UUID16_WRITABLE( HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_SCAN_RESULT_CFG_DESC, GATT_UUID_CHAR_CLIENT_CONFIG,
             LEGATTDB_PERM_READABLE | LEGATTDB_PERM_WRITE_REQ),




         /*Declare command control characteristic used for recieving commands from central device*/
         CHARACTERISTIC_UUID128_WRITABLE( HANDLE_WIFI_INTOR_SENS_NW_SERVICE_CHAR_COMMAND_CONTROL, HANDLE_WIFI_INTOR_SENS_NW_SERVICE_CHAR_COMMAND_CONTROL_VAL,
                 UUID_WIFI_INTRODUCER_CHARACTERISTIC_COMMAND_CONTROL, LEGATTDB_CHAR_PROP_READ | LEGATTDB_CHAR_PROP_WRITE,
             LEGATTDB_PERM_READABLE | LEGATTDB_PERM_WRITE_CMD | LEGATTDB_PERM_WRITE_REQ
             | LEGATTDB_PERM_AUTH_READABLE | LEGATTDB_PERM_AUTH_WRITABLE ),

          //CHARACTERISTIC_UUID128( HANDLE_WIFI_INTOR_SENS_NW_SERVICE_CHAR_COMMAND_CONTROL, HANDLE_WIFI_INTOR_SENS_NW_SERVICE_CHAR_COMMAND_CONTROL_VAL,
           //     UUID_WIFI_INTRODUCER_CHARACTERISTIC_COMMAND_CONTROL, LEGATTDB_CHAR_PROP_READ | LEGATTDB_CHAR_PROP_NOTIFY | LEGATTDB_CHAR_PROP_INDICATE | LEGATTDB_CHAR_PROP_WRITE, LEGATTDB_PERM_READABLE  | LEGATTDB_PERM_AUTH_WRITABLE),
                ...
                /* Declare client characteristic configuration descriptor
                          ...
                          * for bonded devices.  Setting value to 1 tells this application to send notification
                          * when value of the characteristic changes.  Value 2 is to allow indications. */
          CHAR_DESCRIPTOR_UUID16_WRITABLE( HANDLE_WIFI_INTOR_SENS_NW_SERVICE_CHAR_COMMAND_CONTROL_CFG_DESC, GATT_UUID_CHAR_CLIENT_CONFIG,
             LEGATTDB_PERM_READABLE | LEGATTDB_PERM_WRITE_REQ),
...
static void app_button_event_handler( const button_manager_button_t* button, button_manager_event_t event, button_manager_button_state_t state )
{
         ...
         ( event == BUTTON_CLICK_EVENT ) )
    {
        ...
        if (dct_tables != NULL)
        {
            memset((char *)dct_tables->stored_ap_list[0].details.SSID.value, 0, sizeof(dct_tables->stored_ap_list[0].details.SSID.value));

            dct_tables->stored_ap_list[0].details.SSID.length = strlen( CLIENT_AP_SSID );

            strlcpy((char *)dct_tables->stored_ap_list[0].details.SSID.value, CLIENT_AP_SSID, sizeof(dct_tables->stored_ap_list[0].details.SSID.value));

            memset((char *)dct_tables->stored_ap_list[0].security_key, 0, sizeof(dct_tables->stored_ap_list[0].security_key));

            dct_tables->stored_ap_list[0].security_key_length = strlen( CLIENT_AP_PASSPHRASE );

            strlcpy((char *)dct_tables->stored_ap_list[0].security_key, CLIENT_AP_PASSPHRASE, sizeof(dct_tables->stored_ap_list[0].security_key));
...
wiced_result_t scan_result_handler( wiced_scan_handler_result_t* malloced_scan_result )
{
    ...
    if ( malloced_scan_result != NULL )
    {
        ...
        if ( malloced_scan_result->status == WICED_SCAN_INCOMPLETE )
        {
            ...
            sprintf(result_string, "%s", record->SSID.value);
            send_scan_results((uint8_t *)&result_string);//" %-32s ", record->SSID.value
...
static void wifi_scan_manager_thread(wiced_thread_arg_t arg)
{
    do
    {
        ...
        wiced_rtos_get_semaphore(&start_ap_scan_semaphore, WICED_WAIT_FOREVER);
        ...
        /* Start the scan */
            ...
            //wait for bluetooth command to start scanning
            //wiced_rtos_get_semaphore(&start_ap_scan_semaphore);/*< TESTING */
       ...
       /*IF AP SCANNING ENABLED, SET AP SCAN SEMAPHORE SO SCANNING WILL CONTINUE*/
       if (TRUE == ap_scanning_enabled)
       {
           wiced_rtos_set_semaphore( &start_ap_scan_semaphore);
       }
...
static void create_wifi_scan_manager_thread(void)
{
    ...
    result = wiced_rtos_create_thread(&network_scan_manager, WICED_DEFAULT_WORKER_PRIORITY, "Scan Manager", wifi_scan_manager_thread, WICED_DEFAULT_APPLICATION_STACK_SIZE, NULL);
    if (result)
    {
        WPRINT_APP_INFO(("%s: thread creation failed !!!!!!!!!\n",__func__ ));
        return;
...
/*
 * Send out Scan results if client has subscribed to notifications/indications
 */
static void send_scan_results( uint8_t *result_string )
{
    //WPRINT_BT_APP_INFO( ( "\n %s: Scan Result Client's Characteristic configuration:%d\n", __func__, wifi_introducer_hostinfo.characteristic_client_configuration ) );
    /* If client has not registered for indication or notification, no action */
    if ( wifi_introducer_hostinfo.scan_result_characteristic_client_configuration == 0 )
    {
        return;
    }
    else if ( wifi_introducer_hostinfo.scan_result_characteristic_client_configuration & GATT_CLIENT_CONFIG_NOTIFICATION )
    {
        uint8_t *p_attr = result_string;
        //WPRINT_BT_APP_INFO(("wifi_introducer_char_notify_value = %s \n", result_string));
        wiced_bt_gatt_send_notification( wifi_introducer_state.conn_id, HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_SCAN_RESULT_VAL, sizeof(wifi_introducer_char_scan_result_value), p_attr );
    }
}
...
static wiced_bt_gatt_status_t wifi_introducer_gatt_server_write_request_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
    ...
    case HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_SCAN_RESULT_CFG_DESC:
        if ( p_data->val_len != 2 )
        {
            return WICED_BT_GATT_INVALID_ATTR_LEN;
        }
        wifi_introducer_hostinfo.scan_result_characteristic_client_configuration = p_attr[0] | ( p_attr[1] << 8 );
        WPRINT_BT_APP_INFO(("\n wifi_introducer_hostinfo.scan_result_characteristic_client_configuration = %d \n" , wifi_introducer_hostinfo.scan_result_characteristic_client_configuration));
        break;

    case HANDLE_WIFI_INTOR_SENS_NW_SERVICE_CHAR_COMMAND_CONTROL_VAL:

        if (p_attr[0] == 0x01){
            ap_scanning_enabled = TRUE;
            wiced_rtos_set_semaphore( &start_ap_scan_semaphore);
            WPRINT_BT_APP_INFO(("COMMAND RECEIVED:  AP Scanning Enabled"));
        } else {
            ap_scanning_enabled = FALSE;
            WPRINT_BT_APP_INFO(("COMMAND RECEIVED:  AP Scanning Disabled"));
        }
    ...
    {
        WPRINT_BT_APP_INFO(("SSID configured \n"));
...
static wiced_bt_gatt_status_t wifi_introducer_gatt_server_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
    ...
    wifi_introducer_hostinfo.scan_result_characteristic_client_configuration = 0;
